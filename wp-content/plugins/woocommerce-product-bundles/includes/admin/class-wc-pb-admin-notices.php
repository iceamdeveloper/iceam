...
class WC_PB_Admin_Notices {
	...
	public static function update_notice() {
		...
		if ( WC_PB_Install::is_update_pending() ) {
			...
			if ( WC_PB_Install::is_update_process_running() || WC_PB_Install::is_update_queued() ) {
				...
				$status = sprintf( __( '<strong>Woo Product Bundles</strong> is updating your database.%s', 'woocommerce-product-bundles' ), $prompt );

			// Show a prompt to update.
			} elseif ( false === WC_PB_Install::auto_update_enabled() && false === WC_PB_Install::is_update_incomplete() ) {

				$status  = __( '<strong>Woo Product Bundles</strong> has been updated! To keep things running smoothly, your database needs to be updated, as well.', 'woocommerce-product-bundles' );
				...
				$status = sprintf( __( '<strong>Woo Product Bundles</strong> has not finished updating your database.%s', 'woocommerce-product-bundles' ), self::get_failed_update_prompt() );
			}
		...
		// Show persistent notice to indicate that the update process is complete.
			...
			$notice = __( '<strong>Woo Product Bundles</strong> database update complete. Thank you for updating to the latest version!', 'woocommerce-product-bundles' );
	...
	public static function welcome_notice() {
		...
		<p class="sw-welcome-text">
			<?php
				...
				echo wp_kses_post( sprintf( __( 'Thank you for installing <strong>Woo Product Bundles</strong>. Ready to get started? <a href="%s">Click here to create your first bundle</a>.', 'woocommerce-product-bundles' ), admin_url( 'post-new.php?post_type=product&wc_pb_first_bundle=1' ) ) );
			?>
		</p>
		<?php
	...
	public static function update_order_item_stats_notice() {
		...
		if ( WC_PB_Admin_Analytics_Sync::is_order_item_stats_update_queued() ) {
			...
			$notice = __( '<strong>Woo Product Bundles</strong> is updating your historical revenue Analytics data. This may take a while, so please be patient!', 'woocommerce-product-bundles' );
			self::add_notice( $notice, 'info' );

		} else {

			$notice = __( '<strong>Woo Product Bundles</strong> has finished updating your revenue Analytics data!', 'woocommerce-product-bundles' );
