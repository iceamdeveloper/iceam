...
jQuery(document).ready(function($) {
	...
	for(var idx = 0; idx < enabled_currencies.length; idx++) {
		...
		$('select#field_to_edit').on('variable_regular_currency_prices_' + currency + '_ajax_data', function(ev, data) {
			ev.preventDefault();
			return NewVariationPrice('regular', $(this).find("option:selected").attr('currency'));
		});
		// Hook for variations sale prices
		$('select#field_to_edit').on('variable_sale_currency_prices_' + currency + '_ajax_data', function(ev) {
			ev.preventDefault();
