...
class WooCommerce_Memberships_Cancelled_Orders_Notice {
	...
	public function modify_membership_views_list( $views ) {
		...
		$custom_views = [
			'active_cancelled_orders' => '<strong> ' . __( 'Active with Cancelled Orders', 'sensei-wc-paid-courses' ) . '</strong>',
		];
	...
	public function add_admin_notice() {
		...
		<div id="sensei-wc-paid-courses-membership-cancelled-orders-notice" class="notice notice-info is-dismissible"
			...
			<p>
				<?php
				esc_html_e(
					'Sensei with WooCommerce Paid Courses has detected active memberships associated with cancelled orders.',
			...
			<p>
				...
				echo wp_kses(
					__(
						'Prior to Sensei with WooCommerce Paid Courses 3.0, active memberships with cancelled orders
						<strong>did not</strong> provide learners with access to the membership\'s associated courses.
						As of Sensei with WooCommerce Paid Courses 3.0, active memberships (regardless of order status)
			...
			<p>
				<a href="<?php echo esc_url( $view_memberships_url ); ?>" class="button button-primary">
					<?php esc_html_e( 'View Active Memberships with Cancelled Orders', 'sensei-wc-paid-courses' ); ?>
				</a>
			</p>
		</div>
		<?php
