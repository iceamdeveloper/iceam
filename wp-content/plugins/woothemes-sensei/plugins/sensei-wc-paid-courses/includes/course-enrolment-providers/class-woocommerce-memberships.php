...
class WooCommerce_Memberships
	...
	public function get_name() {
		return esc_html__( 'WooCommerce Memberships', 'sensei-wc-paid-courses' );
	}
	...
	public function debug( $user_id, $course_id ) {
		...
		foreach ( $memberships as $membership ) {
			if ( $membership->is_active() ) {
				...
				$messages[ 'active-membership-' . $membership->get_id() ] = sprintf(
					...
					__( '<a href="%1$s">Membership #%2$s</a> has an active status and is providing access to enrollment.', 'sensei-wc-paid-courses' ),
				...
				$messages[ 'inactive-membership-' . $membership->get_id() ] = sprintf(
					// translators: %1$s is URL to edit/view the membership; #%2$s is the membership number.
					__( '<a href="%1$s">Membership #%2$s</a> would provide access to enrollment for this course but it does not have an active status.', 'sensei-wc-paid-courses' ),
					get_edit_post_link( $membership->get_id() ),
					$membership->get_id()
				);
			}
		...
		if ( empty( $memberships ) ) {
			$messages['no-memberships'] = __( 'Learner does not have any memberships that would provide access to enrollment for this course.', 'sensei-wc-paid-courses' );
		}

		if ( $this->is_signed_up( $user_id, $course_id ) ) {
			if ( $has_active_membership ) {
				$messages['signed-up-with-membership'] = __( 'Learner has signed up for this course and has an active membership.', 'sensei-wc-paid-courses' );
			} else {
				$messages['signed-up-without-membership'] = __( 'Learner has signed up for this course but does not have an active membership.', 'sensei-wc-paid-courses' );
			}
		} else {
			if ( $has_active_membership ) {
				$messages['not-signed-up-with-membership'] = __( 'Learner has not signed up for this course but has an active membership so they should be able to sign up from the course page.', 'sensei-wc-paid-courses' );
			} else {
				$messages['not-signed-up-without-membership'] = __( 'Learner has not signed up for this course and is not able to without an active membership.', 'sensei-wc-paid-courses' );
			}
		}
