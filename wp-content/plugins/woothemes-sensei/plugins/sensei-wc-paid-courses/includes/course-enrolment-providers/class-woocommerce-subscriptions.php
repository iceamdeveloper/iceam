...
class WooCommerce_Subscriptions
	...
	public function get_name() {
		return esc_html__( 'WooCommerce Subscriptions', 'sensei-wc-paid-courses' );
	}
	...
	public function debug( $user_id, $course_id ) {
		...
		foreach ( $subscriptions as $subscription ) {
			if ( in_array( $subscription->get_status(), $this->get_active_subscription_statuses(), true ) ) {
				$messages[ 'active-subscription-' . $subscription->get_id() ] = sprintf(
					...
					__( '<a href="%1$s">Subscription #%2$s</a> has an active status and is providing enrollment.', 'sensei-wc-paid-courses' ),
				...
				$messages[ 'inactive-subscription-' . $subscription->get_id() ] = sprintf(
					// translators: %1$s is URL to edit/view the subscription; #%2$s is the subscription number.
					__( '<a href="%1$s">Subscription #%2$s</a> contains a product that would provide enrollment for this course but it does not have an active status.', 'sensei-wc-paid-courses' ),
					$subscription->get_edit_order_url(),
					$subscription->get_id()
				);
			}
		...
		if ( empty( $subscriptions ) ) {
			$messages['no-subscriptions'] = __( 'Learner does not have any subscriptions for products that would provide enrollment for this course.', 'sensei-wc-paid-courses' );
		}
