...
class WooCommerce_Simple
	...
	public function get_name() {
		return \esc_html__( 'WooCommerce Simple Products', 'sensei-wc-paid-courses' );
	}
	...
	public function debug( $user_id, $course_id ) {
		...
		foreach ( $orders as $order ) {
			if ( in_array( $order->get_status(), \wc_get_is_paid_statuses(), true ) ) {
				$messages[ 'active-order-' . $order->get_id() ] = sprintf(
					...
					__( '<a href="%1$s">Order #%2$s</a> has a paid status and is providing enrollment.', 'sensei-wc-paid-courses' ),
				...
				$messages[ 'inactive-order-' . $order->get_id() ] = sprintf(
					// translators: %1$s is URL to edit/view the order; #%2$s is the order number.
					__( '<a href="%1$s">Order #%2$s</a> contains a product that would provide enrollment for this course but it does not have a paid status.', 'sensei-wc-paid-courses' ),
					$order->get_edit_order_url(),
					$order->get_id()
				);
			}
		...
		if ( empty( $orders ) ) {
			$messages['no-orders'] = __( 'Learner does not have any orders for products that would provide enrollment for this course.', 'sensei-wc-paid-courses' );
		}
