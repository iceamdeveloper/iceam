	...
	class Aelia_WC_RequirementsChecks {
			...
			foreach($this->required_plugins as $plugin_name => $plugin_requirements) {
				...
				if(is_array($plugin_info) && ($plugin_info['active'] == true)) {
					if(version_compare($plugin_info['version'], $plugin_requirements['version'], '<')) {
						$message = sprintf(__('Plugin "%s" must be version "%s" or later.', 'wc-aelia-eu-vat-assistant'),
															 $plugin_name,
					...
					$message = sprintf(__('Plugin "<strong>%s</strong>" must be installed and activated.', 'wc-aelia-eu-vat-assistant'),
														 $plugin_name);
				}
		...
		protected function js_url() {
			...
			if($plugin_info == false) {
				trigger_error(sprintf(__('%s - Plugin not found: "%s". ' .
																 ...
																 'class "%s". Please report the issue to support.', 'wc-aelia-eu-vat-assistant'),
															get_class($this),
															$this->plugin_name,
															get_class($this)),
											E_USER_WARNING);
		...
		protected function get_plugin_action_html($plugin_name) {
			...
			switch($plugin_action) {
				case 'install':
					...
					$plugin_action .= __('Install plugin', 'wc-aelia-eu-vat-assistant');
					$plugin_action .= '</a>';
					break;
				case 'activate':
					...
					$plugin_action .= __('Activate plugin', 'wc-aelia-eu-vat-assistant');
					$plugin_action .= '</a>';
					break;
				default:
					// Nothing to do
			...
			$aelia_wc_requirementchecks_params = array(
				'user_interface' => array(
					'plugin_install_prompt' => __('This will install and activate the plugin ' .
																				'automatically. Would you like to proceed?', 'wc-aelia-eu-vat-assistant'),
					'plugin_activate_prompt' => __('This will activate the plugin. Would you like to proceed?', 'wc-aelia-eu-vat-assistant'),
				),
			);
		...
		protected function install_plugin($plugin_url, $plugin_name) {
			...
			$result->messages = array(
				sprintf(__('Installing plugin "%s"...', 'wc-aelia-eu-vat-assistant'), $plugin_name)
			);

			$wp_plugin_info = $this->get_wp_plugin_info($plugin_name);
			if(is_array($wp_plugin_info)) {
				$result->messages[] = __('Plugin is already installed.', 'wc-aelia-eu-vat-assistant');
				$result->status = true;
				return $result;
			}
			...
			if($install_result instanceof WP_Error) {
				$result->status = false;
				$result->messages[] = __('Plugin installation failed. See errors below.', 'wc-aelia-eu-vat-assistant');
				...
				$result->messages[] = sprintf(__('Please try installing the plugin manually. You can ' .
																				 'download it from %s.', 'wc-aelia-eu-vat-assistant'),
																			$plugin_url);
			}
			else {
				$result->status = true;
		...
		protected function activate_plugin($plugin_name) {
			$result = new stdClass();
			$result->messages = array(
				sprintf(__('Activating plugin "%s"...', 'wc-aelia-eu-vat-assistant'), $plugin_name),
			);

			$wp_plugin_info = $this->get_wp_plugin_info($plugin_name);
			if(!is_array($wp_plugin_info)) {
				$result->messages[] = __('Plugin is not installed.', 'wc-aelia-eu-vat-assistant');
				$result->status = false;
				return $result;
			}

			if($wp_plugin_info['active']) {
				$result->messages[] = __('Plugin already active.', 'wc-aelia-eu-vat-assistant');
				$result->status = true;
				return $result;
			}
			...
			if($activation_result instanceof WP_Error) {
				$result->status = false;
				$result->messages[] = __('Activation failed.', 'wc-aelia-eu-vat-assistant');
			...
			$result->messages[] = __('Plugin activated successfully. Please refresh this page to ' .
															 'hide the requirement notices and this message.', 'wc-aelia-eu-vat-assistant');
			$result->status = true;
			return $result;
		}
		...
		public function wp_ajax_install_plugin() {
			...
			if($plugin_info === false) {
				$result['status'] = false;
				$result['messages'][] = __('Invalid plugin specified: "%s".', 'wc-aelia-eu-vat-assistant');
			}
			...
			if(empty($plugin_info['url'])) {
				...
				$result['messages'][] = __('Plugin does not have a URL from which it can be ' .
																	 'downloaded automatically.', 'wc-aelia-eu-vat-assistant');
			}
